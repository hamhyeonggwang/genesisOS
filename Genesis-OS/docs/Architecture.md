...
> **실제 배포 시 전 테이블·enum·함수명에 `genesis_` 접두사 적용** (§7.1 — 공유 Supabase 프로젝트의 기존 스키마와 충돌 방지). 아래 DDL은 접두사 이전의 논리적 모델이며, 접두사 매핑은 1:1이다 (`projects` → `genesis_projects`).
...
### 7.1 Supabase 프로젝트 (확정)

- **프로젝트**: `ruxbuidhvodnfztrdwvf` (hamhyeonggwang's Project, ap-southeast-1)
- 기존에 다른 제품(습관·목표 관리 스키마: `habits`, `goals`, `quarters`, `life_roles` 등)이 이 프로젝트를 공유 중이므로, Genesis OS 테이블은 **`genesis_` 접두사**로 분리한다 (`genesis_projects`, `genesis_project_phases` 등 — enum도 `genesis_phase_name` 등으로 접두).
- 마이그레이션 2건 적용 완료: `genesis_os_schema`(테이블 9종+RLS), `genesis_fix_function_search_path`(트리거 함수 `search_path` 고정).
- Supabase 클라이언트 코드는 테이블명에 `genesis_` 접두사를 그대로 사용한다 — §4 DDL 및 `src/types/domain.ts`의 타입명(`Project` 등)은 변경하지 않고, 접두사는 데이터 접근 계층(`src/lib/supabase`)에서만 다룬다.
